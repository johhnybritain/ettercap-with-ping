...
void set_geoip2_file(char *geoip2_file);
...
void ec_usage(void)
{
   ...
   fprintf(stdout, "\nGeolocation options:\n");
   fprintf(stdout, "  -g, --geoip2 <file>         location of GeoLite2-City.mmdb file\n");
...
void parse_options(int argc, char **argv)
{
   ...
   static struct option long_options[] = {
      ...
      { "ping", required_argument, NULL, 'x' },
      { "interval", required_argument, NULL, 'X' },
...
void set_ping(char *ping)
{
	GBL_OPTIONS->ping = atoi(ping);
}
...
void set_geoip2_file(char *geoip2_file)
{
	GBL_OPTIONS->geoip2_file = strdup(geoip2_file);
}
